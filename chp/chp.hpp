...

{
	...
	q->n = n;
	q->x = (unsigned long **) malloc((2*q->n + 1) * sizeof(unsigned long*));
	q->z = (unsigned long **) malloc((2*q->n + 1) * sizeof(unsigned long*));
	q->r = (int *) malloc((2*q->n + 1) * sizeof(int));
	q->over32 = (q->n>>5) + 1;
	q->pw[0] = 1;
	for (i = 1; i < 32; i++)
         q->pw[i] = 2*q->pw[i-1];
	for (i = 0; i < 2*q->n + 1; i++)
	{
         q->x[i] = (unsigned long *) malloc(q->over32 * sizeof(unsigned long));
         q->z[i] = (unsigned long *) malloc(q->over32 * sizeof(unsigned long));
         for (j = 0; j < q->over32; j++)
         {
                 q->x[i][j] = 0;
                 q->z[i][j] = 0;
...

{
	...
	fclose(fp);
	h->a = (char *) malloc(h->T * sizeof(char));
	h->b = (long *) malloc(h->T * sizeof(long));
	h->c = (long *) malloc(h->T * sizeof(long));
	fp = fopen(fn2, "r");
	while (!feof(fp)&&(c!='#'))
         fscanf(fp, "%c", &c);
	t=0;
